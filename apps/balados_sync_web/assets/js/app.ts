...
function initializeTimelineFeatures(): void {
  // Initialize timeline filter if on timeline page
  ...
  if (timelineContainer) {
    ...
    const maxToasts = 15
    const recentCount = Math.min(recentEvents.length, maxToasts)

    for (let i = 0; i < recentCount; i++) {
      const eventEl = recentEvents[i]
      const eventType = eventEl.getAttribute('data-event-type') || 'activity'
      const username = eventEl.querySelector('[data-username]')?.textContent || 'User'
      const title = eventEl.querySelector('[data-title]')?.textContent || 'an event'

      const messages: Record<string, string> = {
        'play': `${username} started playing ${title}`,
        'subscribe': `${username} subscribed to ${title}`,
        'default': `${username} had activity on ${title}`
      }

      const message = messages[eventType] || messages['default']
